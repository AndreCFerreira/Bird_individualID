...
#Read the data in the case of priorityOne RIFDlogger using the following (adapted from:
...
#Create loop to continuously read from the RFIDlogger
while 1:
    x=readData()#read the data from the logger
    #print(x)
    if len(x)>9:#if the string has more than 9 characters (our tags have 10 characters)
                #take a picture. Setting up this threshold is useful to avoid adding noise from
        ...
        #in this case we defined a time interval of 2 sec
        time.sleep(2)
